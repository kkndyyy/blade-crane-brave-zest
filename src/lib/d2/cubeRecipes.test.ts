...
describe("set amulet transmute", () => {
  ...
  it("replaces transmute gems with the amulet on 6-piece sets", () => {
    const cube = parseTsv(
      [
        cubeHeader,
    ...
    assert.equal(on.skipped, 0);
    const mag = cube.rows.find((r) => getCell(r, cube, "description").includes("매직"))!;
    assert.equal(getCell(mag, cube, "output"), "AmuSetImmortal King");
    assert.equal(getCell(mag, cube, "input 7"), '"amu,mag"');
    assert.equal(getCell(mag, cube, "numinputs"), "7");
    assert.ok(getCell(mag, cube, "description").includes("보석생략"));
    assert.equal(getCell(uniques.rows[1]!, uniques, "code"), "amu");
  });
  ...
  it("adds mag/rar recipes for yupgoolg sample without colliding with ring recipes", () => {
    ...
    assert.equal(on.skipped, 0);
    assert.equal(on.recipes, 72);
    assert.equal(on.uniques, 36);
    assert.equal(cube.rows.length, before + 72);
    assert.equal(uniques.rows.length, uniqueBefore + 36);
    assert.equal(isSetAmuletTransmuteEnabled(cube), true);

    const added = cube.rows.filter((r) => getCell(r, cube, "description").startsWith("형상변환 목걸이"));
    assert.equal(added.filter((r) => getCell(r, cube, "description").includes("Immortal King")).length, 2);
    assert.equal(added.filter((r) => getCell(r, cube, "description").includes("Sigon's")).length, 2);
    assert.equal(added.filter((r) => getCell(r, cube, "description").includes("Destroyed Diablo")).length, 2);
