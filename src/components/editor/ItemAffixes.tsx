...
export function ItemAffixEditor({
  ...
  const skillChoices = useMemo(
    () => listSkillChoices(skills, skilldesc, strings),
    [skills, skilldesc, strings],
  );
  const skillGroups = useMemo(() => groupedSkillChoices(skillChoices), [skillChoices]);
        ...
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            {slots.map((slot) => {
              ...
              const skillHit = isSkillProp(prop) && par ? matchSkillChoice(par, skillChoices) : undefined;
              const skillKo =
                skillHit?.ko ||
                (prop && isSkillProp(prop) && par
                  ? koreanSkillName(strings, { skill: par, skillsTable: skills, skilldescTable: skilldesc })
                  : "");
              const skillSelectValue = skillHit?.skill ?? par;
              return (
                <tr key={slot.prop} className="border-t border-border">
                  ...
                  <td className="px-2 py-1.5">
                    {isSkillProp(prop) && skillChoices.length ? (
                      <select
                      ...
                      >
                        ...
                        {par && !skillHit ? <option value={par}>{par} (직접 입력)</option> : null}
                      </select>
                    ) : (
                      <input
                    ...
                    {skillKo && skillKo !== par && skillKo !== skillHit?.ko ? (
